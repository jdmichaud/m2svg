...
fn render_horizontal(graph: &GitGraph, use_ascii: bool) -> String {
    ...
    let mut branches_needing_bridge: HashSet<String> = HashSet::new();
    {
        ...
        let commit_branch: HashMap<&str, &str> = graph.commits.iter()
                ...
                if let Some(source_id) = commit.parent_ids.get(1) {
                    if let Some(&source_branch) = commit_branch.get(source_id.as_str()) {
                        ...
                        if branches_with_tags.contains(&commit.branch) {
                            ...
                            let source_idx = sorted_branches.iter()
                                .position(|(_, b)| b.name == source_branch);
                            let target_idx = sorted_branches.iter()
        ...
        if let Some(parent_id) = fork_info.get(&commit.id) {
            if let Some(&parent_col) = commit_cols.get(parent_id) {
                if let Some(parent) = graph.commits.iter().find(|c| &c.id == parent_id) {
                    ...
                    let parent_len = if parent.is_merge { parent.id.len() + 2 } else { parent.id.len() };
        ...
        if let Some(source_id) = merge_info.get(&commit.id) {
            if let Some(&source_col) = commit_cols.get(source_id) {
                if let Some(source) = graph.commits.iter().find(|c| &c.id == source_id) {
                    ...
                    let row_diff = if source_row > commit_row { source_row - commit_row } else { commit_row - source_row };
    ...
    for branch in graph.branches.iter().skip(1) {
        ...
        for (idx, branch_commit) in branch_commits.iter().enumerate() {
            if let Some(merge) = graph.commits.iter()
                .find(|c| c.is_merge && c.branch != branch.name && c.parent_ids.contains(&branch_commit.id))
            {
        ...
        let last_len = if last.is_merge { last.id.len() + 2 } else { last.id.len() };
        ...
        if merge_idx + 1 < branch_commits.len() {
            ...
            let last_stretched_len = if last_stretched.is_merge { last_stretched.id.len() + 2 } else { last_stretched.id.len() };
    ...
    let merge_order: Vec<(String, String)> = graph.commits.iter()
                ...
                if let Some(&current_merge_col) = commit_cols.get(merge_id) {
                    if let Some(source) = graph.commits.iter().find(|c| &c.id == source_id) {
                        if let Some(merge) = graph.commits.iter().find(|c| &c.id == merge_id) {
                            ...
                            let source_len = if source.is_merge { source.id.len() + 2 } else { source.id.len() };
                            let row_diff = if source_row > merge_row { source_row - merge_row } else { merge_row - source_row };
                ...
                if let Some(&merge_col) = commit_cols.get(merge_id) {
                    if let Some(source) = graph.commits.iter().find(|c| &c.id == source_id) {
                        if let Some(merge) = graph.commits.iter().find(|c| &c.id == merge_id) {
                            ...
                            if source_row < merge_row && branches_needing_bridge.contains(&merge.branch) {
                                continue;
                            }
                            ...
                            if merge_col > diag_arrival + 3 {
                                ...
                                let needed_source_col = needed_source_end.saturating_sub(source_len);
    ...
    let fork_parent_order: Vec<String> = graph.commits.iter()
                        ...
                        let mut tag_collision_row: Option<usize> = None;
                        {
                            ...
                            for row in (source_row + 1)..merge_row {
                                for &(tag_row, tag_start, tag_end) in &tag_areas {
                                    if row == tag_row && check_x >= tag_start && check_x < tag_end {
    ...
    for commit in &graph.commits {
        if let Some(ref tag) = commit.tag {
            ...
            if branch_row >= 2 {
                ...
                set_char(&mut canvas, commit_center as i32, connector_row as i32, chars.v_line);
            }
        }
    }
