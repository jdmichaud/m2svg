...
use crate::types::{MermaidGraph, Direction as MermaidDirection};
use crate::ascii::types::{AsciiConfig, AsciiGraph, AsciiSubgraph, AsciiNode, AsciiEdge, GraphDirection};
...
fn convert_to_ascii_graph(parsed: &MermaidGraph, config: &AsciiConfig) -> AsciiGraph {
    ...
    let id_to_idx: HashMap<&str, usize> = graph.nodes
        .iter()
        .enumerate()
...
fn ascii_graph_to_svg(
    ...
    for edge in &graph.edges {
        ...
        if let (Some(from_dc), Some(to_dc)) = (from_node.drawing_coord, to_node.drawing_coord) {
            parts.push(render_edge_svg(from_dc, to_dc, from_node, to_node, &edge.text, &graph.config));
        }
    }
